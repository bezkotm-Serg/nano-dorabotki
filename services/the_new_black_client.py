...
async def create_variation(image_url: str, prompt: Optional[str] = None) -> str:
    _ensure_auth()
    _ensure_url(image_url)
    email, password = _get_auth()
    prompt = prompt or _get_default_prompt()
    files = {"email": (None, email), "password": (None, password), "image": (None, image_url), "prompt": (None, prompt)}
...
async def create_alternative_views(image_url: str, prompt: Optional[str] = None) -> str:
    _ensure_auth()
    _ensure_url(image_url)
    email, password = _get_auth()
    prompt = prompt or _get_default_prompt()
