...
def ensure_user(user_id: int, welcome_credits: int = 0) -> Tuple[bool, int]:
    ...
    with _LOCK, _CONN:
        ...
        row = cur.fetchone()
        if row is None:
            _CONN.execute("INSERT INTO users(user_id, credits, welcomed, created_at) VALUES(?,?,?,?)",
                          (user_id, 0, 0, now))
            is_new = True
            credits = 0
        else:
        ...
        if is_new and welcome_credits > 0:
            _CONN.execute("UPDATE users SET credits=?, welcomed=1 WHERE user_id=?", (welcome_credits, user_id))
            _CONN.execute("INSERT INTO transactions(user_id, type, amount, meta, created_at) VALUES(?,?,?,?,?)",
...
def add_credits(user_id: int, amount: int, reason: str) -> None:
    ...
    if amount <= 0:
        return
    with _LOCK, _CONN:
        _CONN.execute("UPDATE users SET credits=COALESCE(credits,0)+? WHERE user_id=?", (amount, user_id))
        _CONN.execute("INSERT INTO transactions(user_id, type, amount, meta, created_at) VALUES(?,?,?,?,?)",
...
def register_payment(provider_id: str, user_id: int, credits: int, amount_minor: int, currency: str) -> None:
    with _LOCK, _CONN:
        _CONN.execute("""INSERT OR IGNORE INTO payments(provider, provider_id, user_id, credits, amount, currency, status, created_at)
                         VALUES('yookassa',?,?,?,?,?,'new',?)""",
...
def mark_payment_applied(provider_id: str) -> Optional[Tuple[int, int]]:
    """Возвращает (user_id, credits) если переведён в applied, иначе None."""
    with _LOCK, _CONN:
        cur = _CONN.execute("SELECT user_id, credits, status FROM payments WHERE provider_id=?", (provider_id,))
        row = cur.fetchone()
        if not row:
            return None
